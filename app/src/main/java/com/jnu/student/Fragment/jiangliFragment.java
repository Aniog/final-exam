...
public class jiangliFragment extends Fragment {
    public static int jiangli_score = 0;
    private RecyclerView tasksRecyclerView;
    private jiangliFragment.TasksAdapter tasksAdapter;

    private ArrayList<Tasks> jiangli_tasks;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        jiangli_tasks = new DataDailyTasks().LoadTasks(this.getContext());
        if(jiangli_tasks.size() == 0)
        {
        ...
        tasksAdapter = new TasksAdapter(jiangli_tasks);
            ...
            case 1:
                ...
                builder2.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        jiangli_tasks.remove(item.getOrder());
                        tasksAdapter.notifyItemRemoved(item.getOrder());
                        new DataDailyTasks().SaveTasks(jiangliFragment.this.getContext(), jiangli_tasks);
                    }
                });
                ...
                checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        ...
                        if (isChecked) {
                            ...
                            jiangli_score = Integer.parseInt(scoreTextView.getText().toString());
                            //添加到已完成任务
                            ArrayList<Tasks> finish_task = new DataFinishTasks().LoadTasks(getContext());
                            finish_task.add(new Tasks(textViewTitle.getText().toString(), jiangli_score));
                            new DataFinishTasks().SaveTasks(getContext(),finish_task);
                            // CheckBox 被选中时的逻辑
                            Toast.makeText(getContext(), jiangli_score +"", Toast.LENGTH_SHORT).show();
                            buttonView.setChecked(false);
                            if (getActivity() != null) {
                                Bundle bundle = new Bundle();
                                bundle.putInt("dailyScore", jiangli_score);
